...
class Preferences(context: Context) {
    ...
    var autoReconnect: Boolean
        get() = vpnPrefs.getBoolean(KEY_AUTO_RECONNECT, true)
        set(value) = vpnPrefs.edit { putBoolean(KEY_AUTO_RECONNECT, value) }
    
    // 上次连接时间
    var lastConnectionTime: Long
        get() = vpnPrefs.getLong(KEY_LAST_CONNECTION_TIME, 0)
        set(value) = vpnPrefs.edit { putLong(KEY_LAST_CONNECTION_TIME, value) }
    
    // 连接次数统计
    var connectionCount: Int
        get() = vpnPrefs.getInt(KEY_CONNECTION_COUNT, 0)
        set(value) = vpnPrefs.edit { putInt(KEY_CONNECTION_COUNT, value) }
    ...
    fun recordConnection() {
        vpnPrefs.edit {
            ...
            putInt(KEY_CONNECTION_COUNT, connectionCount + 1)
        }
    }
    
    // 重置VPN首选项
    fun resetVpnPreferences() {
        vpnPrefs.edit { clear() }
    }
    ...
    // Shadowsocks配置获取方法
    fun getShadowsocksConfig(): ShadowsocksConfig = ShadowsocksConfig(
        locals = ssLocals.value,
        servers = ssServers.value,
        balancer = ssBalancer.value,
        online_config = ssOnlineConfig.value,
        log = ssLog.value,
    ...
    fun updateShadowsocksConfig(config: ShadowsocksConfig) {
        ...
        ssOnlineConfig.value = config.online_config
    ...
    private fun loadShadowsocksConfigs() {
        try {
            ...
            ssPrefs.getString(KEY_SS_ONLINE_CONFIG, null)?.let {
                ssOnlineConfig.value = gson.fromJson(it, OnlineConfig::class.java) ?: OnlineConfig()
            }
    ...
    private fun saveShadowsocksConfigs() {
        ssPrefs.edit {
            ...
            putString(KEY_SS_ONLINE_CONFIG, gson.toJson(ssOnlineConfig.value))
